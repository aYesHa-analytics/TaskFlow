...
export default function TaskCard({ task }: Props) {
  const { deleteTask } = require('../store/taskStore').useTaskStore?.() || { deleteTask: () => {} };
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        <span className="text-xs px-4 py-1 bg-teal-100 text-teal-700 rounded-full font-medium">{task.category}</span>
